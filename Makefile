FILES := $(shell /bin/ls *.go | grep -v babygo)
...
self-test: 
# 1st generation
	echo "build 1st generation"
	go build -o babygo
# 2nd generation
	echo "build 2nd generation"
	cat $(FILES) | ./babygo > tmp/babygo2.s
	as -o tmp/babygo2.o tmp/babygo2.s runtime/runtime.s
	ld -o ./babygo2 tmp/babygo2.o
# 3nd generation
	echo "build 3rd generation"
	./babygo2 < $(FILES) > tmp/babygo3.s
